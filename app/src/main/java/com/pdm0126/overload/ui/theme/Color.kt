package com.pdm0126.overload.ui.theme

import androidx.compose.ui.graphics.Color

// Overload Palette

val OverloadBlack = Color(0xFF000000)      // Fondo absoluto (OLED)
val OverloadDarkGray = Color(0xFF121212)
val OverloadMediumGray = Color(0xFF1E1E1E)
val OverloadLightGray = Color(0xFFBDBDBD)  // Texto secundario / Iconos inactivos

val OverloadRed = Color(0xFF8B0000)
val OverloadRedBright = Color(0xFF9f0000)
val OverloadGold = Color(0xFFB8860B)

val OverloadWhite = Color(0xFFF5F5F5)