...
Se consume la API de ExerciseDB para obtener los metadatos técnicos (instrucciones y biomecánica), integrándose fluidamente con la base de datos local. Esta configuración híbrida asegura que el usuario siempre tenga acceso instantáneo a la guía visual y a sus registros, operando de manera *offline-first* para garantizar el rendimiento.
...
* **API Externa:** Se utiliza ExerciseDB (mediante Ktor y OkHttp) como fuente para la obtención remota de nuevos ejercicios. Los metadatos obtenidos se guardan en Room para enriquecer la biblioteca local de la app a elección del usuario.
...
* **Descripción:** Galería de tarjetas que presentan los sistemas predefinidos de entrenamiento (PPL, Arnold Split, Upper/Lower, Full Body).
* **Lógica de Negocios:** Al elegir un sistema, se carga una guía de información. Por ejemplo, al elegir PPL, se informa al usuario sobre la filosofía del esquema (los empujes enfocan el pectoral, el deltoides anterior y tríceps). El Blueprint funciona como una sugerencia, pero el usuario retiene la última palabra.
...
* **Interacción:** El usuario dispone de un lienzo para añadir sus días de entrenamiento y organizarlos visualmente (ej. asignarles la etiqueta de "Push" o "Legs" en un esquema PPL). La interfaz sugiere un flujo basado en la plantilla elegida, pero el usuario puede alterar este orden libremente o crear esquemas asimétricos (ej. Pull - Push - Legs).
* **Límites Flexibles:** El único límite real estructural del módulo es el tamaño del microciclo, permitiendo establecer un máximo de 6 días de entrenamiento para el microciclo, protegiendo al usuario de construir un modelo insostenible.

### 2.3. Pantalla de Personalización de Rutina (Lienzo de Ejercicios)
* **Descripción:** Vista que representa un día específico de entrenamiento dentro del microciclo, listando sus ejercicios de forma secuencial.
* **Estructura y Flexibilidad:** El usuario se encuentra ante un lienzo en blanco. No existen restricciones reales sobre qué grupo muscular debe trabajarse, cuantos ejercicios por grupo muscular ni en qué orden específico. Si el usuario decide asignar un ejercicio de cuádriceps en un día etiquetado como "Push", el sistema confía en el sentido común del practicante.
* **Sugerencias y Reglas de Seguridad:** * La pantalla debe ofrecer al usuario información técnica (a tráves de, por ejemplo, un botón interactivo ("?")) como recomendaciones de programación avanzada (ej. "Agrupa los ejercicios en bloques musculares alternados para optimizar el rendimiento y evitar fatiga local prematura").
  * Los únicos límites reales son de volumen y fatiga: Se permite un límite global de **9 a 12 ejercicios** por día, y cada ejercicio está topado a un máximo de **5 a 6 series** seleccionables.

### 2.4. Pantalla de Librería de Ejercicios y Detalle
* **Descripción:** Catálogo maestro alimentado tanto por la base de datos local (Room) como por la API externa (Ktor), utilizado para llenar el lienzo de la rutina.
